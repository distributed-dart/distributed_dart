...
// Prefixes a bytelist object with the size of the json string
// size prefixe is a 64 bit integer, encoded as Uint8List(8)
StreamTransformer _byteListEncoder() {
  return new StreamTransformer.fromHandlers(
      handleData: (var data, EventSink sink) {
...
StreamTransformer _byteListDecoder() {
  const int PKG_SIZE_LENGTH = 8; // 64 bit = 8x8 bits
  ...
  return new StreamTransformer.fromHandlers(
      handleData: (var data, EventSink sink) {
        ...
        while (idx < data.length) {
          ...
          if(remaining == -1) {
            if (sizeBuffer != null) {
              // We did not got all data to get the package size from last 
              // time we run handleData. Therefore we now try get the missing 
              // parts of the length.
              ...
              if (data_remaining < neededData) {
                // Well this should never happen but sometimes it is possible 
                // to get unlucky. In this case we recieve a package there are 
                // too small to contain the remaining of the package size.
              ...
              if (data_remaining < PKG_SIZE_LENGTH) {
                // Well, there are not enough space left in the network package 
                // to contain the package size. We create a new buffer and wait 
                // for rest to get the package size.
...
StreamTransformer _dataConverter(final Converter converter) {
  return new StreamTransformer.fromHandlers(
      handleData: (var data, EventSink sink) {
        sink.add(converter.convert(data));
  });
}
